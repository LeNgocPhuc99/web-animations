...
const AdvAnimationLabs = () => {
  const activeId = useActiveSection(advSectionIds);

  return (
    <div className="min-h-screen bg-bg-main font-sans text-sm leading-relaxed text-text-base">
      <Navbar
        links={advSectionLinks}
        logo="Adv Animations"
        logoColor="bg-adv"
      />
      <main className="mx-auto w-full max-w-295 px-6 pb-18 max-sm:px-4">
        <section className="relative overflow-hidden py-14 text-center">
          ...
          <div className="relative z-10">
            <div className="lab-fade-0 mb-3.5 font-mono text-xs uppercase tracking-[0.12em] text-adv">
              // kỹ thuật animation nâng cao
            </div>
            <div className="lab-fade-3 flex flex-wrap justify-center gap-2">
              {advAnimationPill.map((pill) => (
        ...
        <div className="mt-6 grid grid-cols-[250px_minmax(0,1fr)] items-start gap-8 max-lg:grid-cols-1">
          ...
          >
            ...
            <nav className="lab-section-list grid gap-1 max-lg:grid-cols-2 max-sm:grid-cols-1">
              {advAnimationLessons.map((lesson) => (
                <a
                  className={cn(
                    sidebarLinkClass,
                  ...
                  key={lesson.id}
                >
                  <span className="font-mono text-[10px] text-adv">
                    {lesson.num}
                  </span>
                  {lesson.title}
                </a>
...
export default AdvAnimationLabs;
